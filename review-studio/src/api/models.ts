...
export interface ReviewApi { listRuns():Promise<Run[]>; listFindings():Promise<Finding[]>; listArtifacts():Promise<Artifact[]>; getPassport(runId:string):Promise<EvidencePassport>; getGraph():Promise<{nodes:GraphNode[];edges:GraphEdge[]}>; getGraphQuery(query:GraphQuery,id?:string):Promise<GraphQueryResult>; listTemplates():Promise<Template[]>; listBaselines():Promise<Baseline[]>; listRevisions():Promise<Revision[]>; listPublications():Promise<Publication[]>; streamProgress?(runId:string,onEvent:(event:{type:string;progress?:number;message?:string})=>void,signal?:AbortSignal):Promise<void>; }
