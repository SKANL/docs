...
- [ ] 3.1 RED: `tests/integration/test_cli_core.py` (+ existing `test_cli_collection/docx/template/doc/asset/context/section.py`) — assert each command still reachable post-split.
- [ ] 3.2 GREEN: split `cli/main.py` into `cli/commands/*_app.py` sub-Typer apps by concern (pipeline, assets, ingest placeholder, template/doc/collection/context/section — mirrors existing command groups).
- [ ] 3.3 GREEN: delete root `main.py` (dead entrypoint).
- [ ] 3.4 Verify: `uv run pytest` green; manually confirm `--help` lists identical command surface.
- [ ] 3.5 Rollback: revert PR3; CLI entrypoint reassembled from sub-apps, no state impact.
