...
Note: no prior formal spec.md exists for this domain; "Previously" lines describe current hardcoded behavior in `src/docs/domain/pipeline.py`, `cli/main.py`, and related modules, which this delta replaces.

## MODIFIED Requirements

### Requirement: Data-Driven, Format-Agnostic Stage Plan

The system MUST derive the ordered list of pipeline stages (`prep`, `assemble`, `all`) from configuration/registry keyed by target output format, rather than a single hardcoded stage list. Stage names, ordering, and fail-fast flags MUST contain no format-specific or "tesina" identifiers in the domain layer.
(Previously: `pipeline_stage_plan` returned fixed, hardcoded module-level stage tuples with a single implicit DOCX/"tesina" assumption and no external configurability.)

#### Scenario: Unknown stage_set still rejected

- GIVEN a stage_set not in `prep`, `assemble`, `all`
...
- WHEN the `assemble` stage plan is requested
- THEN the returned stages match the docx-specific configuration in dependency order

#### Scenario: No hardcoded format identifiers remain

- GIVEN the `domain/pipeline.py` module
- WHEN inspected
- THEN it contains no literal "tesina" or DOCX-only sentinel identifiers
...
The system MUST declare `docxcompose` (plus `filetype` and `opendataloader-pdf`) as explicit dependencies in `pyproject.toml`, and MUST NOT silently swallow exceptions in `filesystem_source_repository.py`.
...
- THEN it is logged or re-raised with context, not silently caught and hidden as a bare `None`/empty-string fallback
