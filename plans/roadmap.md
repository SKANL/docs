...
|---|-------|--------------|------|-------|
| 10 | Format Audit (DOCX) | 2016-2184 | medium | `format_audit_docx`/margin EMU helpers/docx parts helpers. New dependency: `python-docx` |
| 11 | DOCX Assembly | 2297-2862 (~565 lines) | large — likely splits into "core" + "layout/TOC/numbering" when planned in detail | `build_docx` + ~15 helpers (structure parts, cover doc, main doc, asset embedding, assemble_structure, page numbering, TOC, styles) |
...
**Total remaining: 6 slices** (10-15). Slice 11 (Assembly) may split into two when planned in detail given its size, which would make the real remaining count 6 or 7.
