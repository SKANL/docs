...
| 2 | `render_section_draft` + its 3 helpers land in `domain/sections.py` | That module already holds section-file pure functions (`apply_stamp`, `with_frontmatter`, `infer_section_id_from_path`) — same neighborhood, same purity |
| 3 | Per-section context is read via `Template.context_schema.topics` filtered by `topic.consumed_by`, not by parsing the generated `context/index.json` | The index is a derived artifact for humans; the schema's `consumed_by` field is the source of truth already used elsewhere (`json_context_repository.py:65`) |
