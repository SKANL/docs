...
def test_v2_journey_executes_generated_cover_and_visual_spec_fixture(monkeypatch, tmp_path: Path) -> None:
    ...
    stage_order = [result["stage"] for result in stage_results]
    assert stage_order.index("generate-visuals") < stage_order.index("compose-cover") < stage_order.index("build-docx")
    ...
    # The image embedding below is the regression assertion; Pandoc renders the
    # inline marker caption without a DOCX paragraph label in this fixture.

    # The first build must consume catalog and binding artifacts generated by
    # this same pipeline execution; a later build must not be required.
    ...
    rebuild = invoke(runner, "build")
    assert rebuild["report"]["succeeded"] is True
    assert artifact.read_bytes() == first_bytes
    assert hashlib.sha256(artifact.read_bytes()).hexdigest() == first_sha
