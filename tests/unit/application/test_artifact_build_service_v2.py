...
def test_rejects_missing_empty_or_outside_scratch_outputs(tmp_path: Path, build) -> None:
    ...
    with pytest.raises(ArtifactBuildErrorV2):
        service.build(
            document_id="active",
            config={},
            output_format="html",
...
def test_rejects_an_unreadable_output_from_the_injected_validator(tmp_path: Path) -> None:
    ...
    with pytest.raises(ArtifactBuildErrorV2, match="cannot reopen"):
        service.build(
            document_id="active",
            config={},
            output_format="html",
...
def test_rejects_a_document_id_that_resolves_outside_scratch(tmp_path: Path) -> None:
    ...
    with pytest.raises(ArtifactBuildErrorV2, match="outside the v2 render scratch directory"):
        service.build(
            document_id="../escaped",
            config={},
            output_format="html",
...
def test_rejects_lexically_escaping_scratch_before_creating_it(tmp_path: Path) -> None:
    ...
    with pytest.raises(ArtifactBuildErrorV2, match="outside the configured scratch parent"):
        service.build(
            document_id="active",
            config={},
            output_format="html",
