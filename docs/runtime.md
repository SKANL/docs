...
artifacts isolated; the alternate pipeline is a finite compatibility bridge, not a
...
| `docs document build` | `docs document build --format ...` | Writes verified requested formats under `output/v2/`, not unverified output. |
...
Legacy pipeline commands are removed
...
once the document/workspace inventory has no remaining legacy consumers.
...
V2 uses the existing workspace and template data model but has a separate output and provenance boundary. The `document` command is the only public pipeline surface. There is no automatic promotion from `output/v2/` to `output/final/`; choose publication explicitly. Public stage-backed boundaries can also be executed independently with `--pipeline`, for example `docs document build --pipeline document-package` and `docs document build --pipeline document-publish` after a verified build.
