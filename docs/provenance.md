...
V2 provenance is separate from the current ledger. It is content-bound evidence for a specific run, manifest, and artifact byte set.

## Ledger

The native ledger is `runs/provenance.json` and has `runs` and `attestations` maps. Each run records a stable run id, input paths and SHA-256 values, and output paths and SHA-256 values. Paths inside the document root are stored relative to the ledger directory; external paths are stored resolved.
